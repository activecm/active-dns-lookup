...
def print_line(output_line, out_h) -> None:
...
def data_line_of(field_name_list: List[str], field_value_list: List[str], input_type: str, cl_args: Dict, zeek_file_path) -> str:
...
def print_sim_tsv_header(line_dict: Dict, requested_fields: List, cl_args: Dict, output_h):
...
def process_log_lines(log_file: str, original_filename: str, requested_fields: List[str], cl_args: Dict):
	...
	with open(log_file, 'r', encoding='utf8') as log_h:
		...
		for _, raw_line in enumerate(log_h):							#_ is the line count
			...
			if (cl_args['allheaders'] or cl_args['allminheaders'] or (cl_args['_one_hdr'] and process_log_lines.data_line_seen is False)) and (cl_args['tsv'] and file_format == 'json' and process_log_lines.tsv_headers_printed is False):	# type: ignore # pylint: disable=too-many-boolean-expressions
...
def link_or_copy(source_dirname: str, source_basename: str, destdir: str, dest_relative_file: str):
